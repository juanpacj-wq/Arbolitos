// lib/screens/crear_arbol/crear_arbol_screen.dart
...
class CrearArbolScreen extends StatefulWidget {
  ...
  @override
  State<CrearArbolScreen> createState() => _CrearArbolScreenState();
}

class _CrearArbolScreenState extends State<CrearArbolScreen> {
  ...
  Future<void> _selectImages() async {
    try {
      ...
      if (selectedImages.isNotEmpty) {
        ...
        if (_imagenes.length + imageFiles.length > 5) {
          // ignore: use_build_context_synchronously
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Puedes seleccionar máximo 5 imágenes'),
              backgroundColor: AppTheme.errorColor,
            ),
          );
          
          // Tomar solo las que faltan para llegar a 5
          final int espacioDisponible = 5 - _imagenes.length;
          if (espacioDisponible > 0) {
            setState(() {
              _imagenes.addAll(imageFiles.take(espacioDisponible));
            });
          }
        } else {
          setState(() {
            _imagenes.addAll(imageFiles);
          });
        }
      }
    } catch (e) {
  ...
  // Guardar árbol
  Future<void> _guardarArbol() async {
    ...
    try {
      ...
      // Crear nuevo árbol
      final arbol = await arbolProvider.createArbol(
      ...
      if (arbol != null && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Árbol creado correctamente'),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Crear Árbol Conmemorativo'),
      ),
      body: _isLoading
